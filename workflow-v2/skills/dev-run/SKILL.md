...
Distinguish a real task/scope conflict from wording differences or same-task recovery. For the latter reuse the Goal or apply Runtime Goal step 5; `/goal clear` is not a universal remedy for a blocked Goal. Goal replacement never changes the approved product boundary or releases Flow pauses, host refusals or budgets.

- Explain the actual old and proposed new objectives in the conversation language, that old unfinished work/checkpoints remain unfinished and preserved, and that replacing the objective resets host Goal usage accounting, not incurred usage or still-applicable limits. Ask once which task to pursue. An explicit human instruction to replace the old goal with the stated new task is already that decision; an ambiguous "继续" is not. Clarify any unresolved cross-task budget in this same interaction; do not invent a fresh allowance. Bind the decision to the observed thread, old reference/objective and proposed objective in the conversation/evidence.
- Inspect available capabilities, not just protocol documentation. `flowctl goal record` only records real observations; `create_goal` cannot replace an unfinished Goal and `update_goal` cannot cancel or change its objective. Use `thread/goal/set` only through a genuinely available, verified, host-permitted capability after explicit replacement direction; this skill does not add a bridge or permit circumventing tool restrictions. Re-read immediately before a lifecycle call: the approved old target may be replaced, the intended new target may be verified/reused, but a third target requires clarification. Read/write is not an atomic compare-and-set guarantee.
- With no replacement capability, combine the conflict explanation, selected/proposed task and manual recovery action in one message. Instruct the human to execute `/goal clear` in this same Codex conversation and then ask to continue the stated new task. Clearing plus explicit continuation of that task satisfies the choice without a separate confirmation question. Do not run slash commands in a shell, start a replacement Codex process, edit host state or mark the old Goal complete. Do not recommend clearing a target whose identity/result cannot be verified, or use clearing to evade a still-binding pause/limit/refusal.

```text
/goal clear
```

- After manual clearing, recheck with `get_goal`; only a verified empty Goal permits `create_goal` for the already selected objective. If the intended new Goal already exists, verify/reuse it instead. Read back after set/create, including after a timeout or error: a verified new Goal is recorded, a verified old conflicting Goal permits the manual recovery suggestion, an empty state permits the normal creation path, and an unreadable or unexpected state remains unconfirmed rather than triggering blind clear/retry. Only pass token_budget when explicitly requested and applicable to this task. Never claim activation from a reply or controller record alone.
- Record the verified new reference through Runtime Goal step 3, retaining old reference history; history is not a full usage ledger. Resume the matching issue/worktree/controller and assess scope changes through existing authorization/routing, not by inheriting an unrelated task's controller or approvals. No goal-only reset of stages, coder or reviews. If durable activation is unavailable, disclose it once and continue explicitly human-requested, safe authorized same-turn work under steps 5–6; Goal uncertainty does not itself justify another business gate or automatic business BLOCKED.
...
这些资源可能包括测试数据库、缓存、搜索、消息队列、对象存储；仅做本议题所需读写。集成测试会保留真实测试请求写入的业务数据，只清理本轮启动的进程、容器、端口、锁和非证据临时文件；项目规则或你明确要求清理的数据除外。

The run must retain real test-environment business writes and record their namespace or identifiers; retained writes are expected evidence of simulated real execution, not an incomplete cleanup.
...
```text
idea or unresolved requirement -> $dev-requirement (which uses $dev-brainstorm)
READY_FOR_INTENT requirement.md -> $dev-intent
confirmed intent.md -> start at `$dev-roadmap`
confirmed roadmap.md plus one selected MILESTONE-* -> $dev-spec
existing spec.md -> $dev-spec review, then $dev-plan (actual accepted Spec handoff can enter Plan directly)
existing plan.md -> $dev-plan review, then $dev-code (actual accepted Plan handoff can enter Code directly)
complete code handoff -> $dev-integration
...
When the user supplies a sufficiently explicit requirement and explicitly wants to skip discussion, perform a raw requirement bootstrap instead of `$dev-brainstorm` or the requirement agent swarm. Faithfully normalize the user's complete statement into a canonical `requirement.md` accepted by `$dev-intent`: issue, immutable human source snapshot, one stated candidate direction, user-provided evidence, known objections, unknowns, coverage limits, content revision/digest, and `status: READY_FOR_INTENT`. Mark provenance `HUMAN_BOOTSTRAP` and the skipped substages. Show the captured requirement to permit correction, but must not invent product decisions, evidence, consensus, PMS facts, or agent conclusions. Any unresolved product choice or unclear success boundary disqualifies bootstrap and routes to `$dev-requirement`.

## Run and resume

At recovery, run flowctl resume before deciding progression and apply Review occupancy recovery in `../../flowctl-contract.md`. The controller also reconciles occupancy on successful binding changes, so this is not startup-only compatibility. Inspect returned recovery_disposition records: they release obsolete binding occupancy, not prove execution stopped. Query available task/process handles before conflicting launches or handoff; preserve uncertain execution and known findings, continue safe diagnosis without guessing termination. Never clear real human/safety pauses by reconciliation or fabricate a terminal review result.

Record human-selected iBrain through `flowctl review select-external`; it needs no Cursor attempts. Record explicit GPT-only restrictions via `flowctl review degrade --lane external --basis human --reason <instruction>`. Reuse human constraints on resume and subsequent reviewed stages. Unavailability can be recorded with basis unavailable for the current artifact. Preserve receipts/findings and current host restrictions; no new reviewer permission gate.
...
For an unrelated/ambiguous existing runtime Goal, apply Goal conflict recovery above, including the verified same-conversation `/goal clear` action when needed; do not ask the user to edit Goal/controller fields or falsely complete the old task. Missing Goal capability is a one-time “本次不能保证跨轮自动续跑，我仍会继续当前可执行步骤” notice, not a confirmation gate. When a real host refusal requires action, name the blocked operation and the permitted host action; do not invent a recovery command or keep requesting already-given Flow consent.
