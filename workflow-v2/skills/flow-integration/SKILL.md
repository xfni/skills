...
When returning `FLOW_RUN_ROUTE_BACK`, or a `FLOW_RUN_BLOCKED` carrying replay gaps, include the complete digest-bound `integration_results`, never a caller-supplied `open_gaps` subset. The supplied status must match the signal: `FAILED` for `FLOW_RUN_ROUTE_BACK`, `BLOCKED` for `FLOW_RUN_BLOCKED`. For route-back and replay gaps, Flowctl revalidates the scenario set and summary against the current Plan, then the controller derives durable gaps. Environment-only BLOCKED signals follow the minimal cause/evidence/resume-condition rule above, not this full-result requirement.
