...
The installed grilling path must produce equivalent records and must not turn model pressure into a decision. Do not repeat a question with no new evidence. Implementation-only uncertainty belongs in `Remaining Unknowns`. If an intent-changing fact or authority choice cannot be resolved, update `requirement.md` to `status: BLOCKED`, increment revision, record the named blocker and required owner/evidence, do not write `intent.md`, and stop with the appropriate resume condition; return to `$dev-requirement` only when new requirement exploration is actually needed.
