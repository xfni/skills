...
Carry every `EXTERNAL_REVIEW_GAP` into the final report. In orchestrated mode return the handoff unaccepted; the root alone accepts it once. Missing historical tuples or auxiliary metadata are not human unlock gates. A failed/unknown real test is not PASS, and production replay safety/cleanup remain required.

Before replay, require the active milestone and TESTCASE plus the currently approved Requirement/Plan chain. The manifest must name `milestone_id`, `scenario_id`, `plan_revision`, and `plan_digest`; its remaining canonical JSON must match the scenario's Plan-approved `production_dependency.replay_manifest_digest`. Both validate and run reject earlier stages, absent Plans, undeclared/non-production scenarios, and drift before acquisition. Direct mode without that chain cannot acquire production-derived data through replay.

Treat unresolved `BLOCKED_CLEANUP` or `INTERRUPTED` evidence as a controller-wide blocker even if all tests pass or authorization changes to skip replay. Run `flowctl replay cleanup --binding-id` with the original binding and current state revision to obtain a recorded cleanup proof. Failed proof stays blocked; never replace it with a signal or a claim that cleanup succeeded. For any new Plan contract, include complete `integration_results` in the Integration artifact; registration, handoff, and resume reject omitted results.
...
Consume the active decision through its `production_replay authorization ID and revision`. Under `SANITIZED_LOCAL_REPLAY`, call flowctl with the Plan-pinned controller-validated replay operation manifest before acquisition and use only its single-use binding. Under `SKIP_PRODUCTION_REPLAY`, still execute every unaffected `TESTCASE-*`. The stage must not ask for replay authorization again while the identity, decision, and manifest scope remain valid; a changed choice or expanded manifest uses `flowctl authorization amend`. Only a scenario whose approved Plan trace proves production-derived data is necessary and synthetic or isolated test-environment data is insufficient may become `SKIPPED_AUTHORIZED_REPLAY`. Create one durable open `PRODUCTION_REPLAY_GAP` per skipped scenario with scenario IDs, missing assurance, reason, owner, and remediation. Any `FAILED` result takes precedence, then any `BLOCKED` result; authorized skips never mask either. All `PASSED` remains `PASSED`; only `PASSED` plus authorized skips, or skip-only, becomes `COMPLETE_WITH_DEFECT`. A skip-only outcome must carry an explicit zero-executed warning. Carry every gap through the handoff, resume state, and final report, and never make a clean completion claim while one remains open.

Load the Plan's strict `integration_scenarios` machine contract and bind the submitted result to its current approved Plan digest and revision. The submitted IDs must equal that complete expected scenario set exactly; no missing unaffected scenario and no extra scenario is allowed. Derive production dependency, reason, missing assurance, owner, and remediation only from that digest-bound Plan contract, never from scenario-supplied `plan_trace` or prose. Record every scenario status in the aggregate so flowctl can recheck counts, warnings, gaps, and precedence. A verified legacy Plan without the machine contract remains a clean historical checkpoint, but it cannot authorize a skip or a new Integration results contract; route to `$flow-plan` for a Plan revision and re-review first.

When returning `FLOW_RUN_ROUTE_BACK` or `FLOW_RUN_BLOCKED` from Integration, include the complete digest-bound `integration_results`, never a caller-supplied `open_gaps` subset. `FAILED` status must match the signal `FLOW_RUN_ROUTE_BACK`, and `BLOCKED` status must match the signal `FLOW_RUN_BLOCKED`. Flowctl revalidates the complete scenario set and summary against the current Plan, then the controller derives the durable gaps; omitted results, missing or extra gaps, and self-reported gap metadata are rejected without changing failure/blocker priority.
