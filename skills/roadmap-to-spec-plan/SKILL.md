...
**REQUIRED SUB-SKILL:** Use `cursor-review` for every `backend: cursor` connection check and review run. This workflow still owns the design profile, brief, ordering, review ledger, and finding disposition.
...
After external review is authorized, invoke `$cursor-review` with the repository/worktree and a brief built from approved scope, baseline version, relevant paths, decisions, acceptance IDs, the `design` profile, and the required finding schema. Use its default `grok-4.6`/`high` configuration unless this workflow explicitly selects an override. Its exit code proves only receipt of a non-empty report, not approval. Record agent/run IDs, covered version, report, findings, and dispositions in the review ledger. If it returns `INCOMPLETE`, report the assurance gap rather than claiming equivalent coverage.
