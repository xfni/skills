...
Before production acquisition/local tests, use the active production-data decision and approved Plan scenario (or the confirmed Direct charter). Use project runners and project data policy; no Flow sanitizer, trusted profile or replay manifest is required. Record source, actual input snapshot digest, execution command, local real requests and results without copying data values into evidence. Keep production-derived datasets outside the original worktree so neither the frozen reviewer view nor Git can include them; Git ignore and a private-looking filename are not review exclusions. Use project-approved local paths and keep data values out of model-visible output. Do not infer execution from a file's presence or a model claim.

Clean up the temporary services and test side effects owned by this run. An actual unresolved unsafe side effect remains a safety obstacle; a legacy sanitizer/raw-data cleanup annotation alone does not block Flow. Do not delete an existing dataset merely to obtain a cleanup receipt. New Plan contracts still require complete `integration_results` at registration, handoff and resume.
