...
| [cursor-review](./skills/cursor-review/) | 通过 Cursor SDK 独立探索过滤后的完整冻结工作树。 | 显式调用、摘要校验、受限只读工具；结果仅通过 stdout/API 返回。 |
...
| 审阅后端 | Cursor SDK 审阅冻结视图；两次可重试运行失败后，Flow 可切换可信 iBrain/glm-5.3。 |
...
`$cursor-review` 使用控制器绑定的 schema-version-2 请求、`--workspace` 私有过滤视图和 `--expected-request-digest`。将 cursor-sdk 安装到 `~/.codex/runtime/cursor-review`，密钥保存在 `~/.cursor-review/API_KEY`。控制器以隔离 Python 执行固定适配器字节，校验原工作树与私有视图后才采纳 stdout/API 报告。仅提供受限的列表、读取、搜索工具；不自动提交，reviewer 不写工作树。Flow/Cursor 调用无需额外授权，可信 iBrain 也独立免授权；宿主权限和生产回放授权仍分别遵守。[详见审阅契约](workflow-v2/review-contract.md)。
