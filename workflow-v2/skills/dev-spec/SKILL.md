...
Intent remains authoritative for Scope, Non-goals, Invariants, and Success Signals. Roadmap defines this milestone's delivery boundary. Repository investigation establishes existing behavior and interfaces. If specification requires changing product intent, return to `$dev-intent`; if it requires changing milestone boundaries or order, return to `$dev-roadmap`.
...
Report current Spec, available inputs, milestone and controller-recorded review receipts/gaps. Under `FLOW_RUN_CONTEXT`, return the unaccepted `FLOW_RUN_HANDOFF` with `next_stage: flow-plan`; otherwise stop and suggest `$dev-plan`. Do not create a Plan or implementation here.
