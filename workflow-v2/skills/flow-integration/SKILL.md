...
Clean up the temporary services and test side effects owned by this run. An actual unresolved unsafe side effect remains a safety obstacle; a legacy sanitizer/raw-data cleanup annotation alone does not block Flow. Do not delete an existing dataset merely to obtain a cleanup receipt. Completion needs actual terminal results, not perfect Plan machine metadata; recording a failure needs only available obstacle evidence.
