...
External review evidence records the controller-issued package binding ID, backend/stage/artifact, prompt/view digests and frozen source snapshot. Every attempt gets a fresh binding. Production-data tests record the scoped authorization ID/revision, input source/digest and actual execution/results, or an active skip decision. No sanitizer/profile/replay-manifest tuple is required. Neither artifact prose nor review trust broadens production authority.
