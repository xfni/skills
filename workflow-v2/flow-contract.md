...
Before that final confirmation, `$dev-run` and `$dev-requirement` provide the issue's foreseeable authorization forecast using targeted project evidence. The authorization forecast is Agent-maintained evidence: it helps one human decision cover bounded production reads, declared private/model receivers, test resources, local/worktree actions, budgets and exclusions, then drives a non-destructive preflight while the human is available. It must not become a new controller schema, admission prerequisite, artifact digest dependency or historical completeness gate. The production-replay decision remains the only dedicated structured data choice; retain the forecast and its human source in the existing Requirement/Direct evidence.

Do not omit or invent a foreseeable target merely because configuration is runtime-injected. Record `UNRESOLVED(runtime:<source-or-key>)`, the inspected source and a verifiable safe predicate. Resolution inside that confirmed predicate proceeds; an unresolved receiver/data class without such a predicate is not authorized for transfer. Carry the production-replay choice as an inherited decision and refine it with source/operation/caps rather than asking the same yes/no again.

The forecast does not replace host sandbox, network or credential permission and cannot authorize production writes, deployment, destructive operations or Git remote changes by implication. A later need inside the confirmed boundary continues automatically. A real expansion requests only the affected delta and preserves unaffected authority. Older runs without this evidence continue from actual current facts; they are not routed back merely to manufacture it.
