...
## Connectivity harness and semantic oracle

Any integration script, runner, probe, driver, or test-only program is a **connectivity and evidence-collection harness**. It may start or stop the local application, wait for readiness, send the real request, preserve correlation identifiers, collect the response, query or export relevant logs/audit/storage observations, enforce bounded timeouts, and report whether those operations were executable. It must not determine business correctness. Its assertions are limited to mechanical preconditions for executing and observing the scenario, such as process startup, endpoint reachability, protocol exchange, parseable evidence, and successful evidence collection.

A harness exit code `0` proves only that its scoped connectivity and collection operations completed; it does not prove that the response or downstream behavior is correct. A non-zero exit proves only the named mechanical operation failed and must be classified from evidence as an implementation failure, environment blocker, or unresolved diagnostic gap. The harness must not emit a business `PASSED` or `FAILED` verdict, encode expected business values as its terminal decision, or convert HTTP status, response fields, log patterns, row counts, audit events, cache values, index contents, or message state directly into the Flow result. It may capture all of those observations unchanged for evaluation.

The Agent owns the semantic verdict for every `TESTCASE-*`. It reads the approved Spec/charter decision rule, then compares the collected response, logs, audit records, database state, search/cache/queue/object state, and bounded asynchronous outcome against that rule. Use at least the evidence sources necessary to establish the actual cross-boundary effect; do not require every source when the contract does not need it. Explain which observation proves or contradicts each expected result and cite its timestamp, correlation/run/scenario identifier, command or evidence location. Evidence missing from a successful harness run remains unverified and can never become `PASSED`; continue bounded observation or classify the scenario as `FAILED`/`BLOCKED` from the actual cause.
...
The Agent uses the returned response plus appropriate probe, logs, audit records, state inspection, or bounded wait for returned or persisted data to decide the result. Observability is evidence, not a substitute for starting the real service and sending real requests, and no harness assertion substitutes for the Agent's semantic comparison. Record readiness and observation deadlines; an unmet condition ends `FAILED` or `BLOCKED` according to whether behavior or environment caused it. Run normal, error, boundary, permission, compatibility, and recovery cases required by the governed Spec or confirmed direct charter; do not add unrelated exploratory scope. After the run, stop the local service and perform only the minimal runtime cleanup. Retain real test-environment business writes and record where they can be inspected.

The stage may add or update minimal test-only code, integration tests, drivers, observers, fixtures, and evidence only in paths explicitly allowed by the approved Plan or confirmed direct charter. Such test code may drive requests, prepare isolated data, wait, collect or serialize observations, and verify mechanical connectivity, but it cannot decide the business result. It must not change business-logic semantics, production source, production defaults, authorization, transactions, or the real boundary under test. Compare its diff to the frozen snapshot before execution. If production implementation must change, stop and route the work outside this stage. Diagnose failures without production edits; after an upstream fix, obtain a new governed handoff or reconfirm a new direct snapshot and rerun affected scenarios.
